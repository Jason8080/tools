 ...
 * </p>
 * <p>
 * 使用 raw type extends（{@code ImPublisher} 无泛型参数），由 TopicRegistry 通过
 * wildcard capture 保证类型安全。
 * </p>
 * <p>
 * 自定义发布器应继承 {@link ImPublisher}，并通过 {@link cn.gmlee.tools.im.spi.factory.PublisherFactory} 创建。
 * </p>
 *
 * @since 5.6.0
 */
...
class DefaultPublisher extends ImPublisher {
    ...
    /**
     * 供 {@link TopicRegistry} 工厂方法使用，延迟解析 Repeater + 自定义路由键 + 组合器 + ID 生成器.
     *
     * @since 5.7.0
     */
    DefaultPublisher(String topic, Supplier<Repeater> repeaterSupplier, List<String> routingKeys,
                     RoutingKeyComposer composer, EventIdGenerator idGenerator) {
        super(topic, repeaterSupplier, routingKeys, composer, idGenerator);
    }
}
